...
<title>Seller pending services</title>
</head>
<body>
			...
			<div class="row">
				<table class="table table-dark col-md-12">
					...
					<tbody>
						...
						<%
						List<BookService> list = BookServiceDao.getSellerPendingServiceList(s.getId());
						%>
						<%
						...
						Customer c1 = CustomerDao.getCustomerDetailsById(s1.getC_id());
						%>
						<%
						Service ser = ServiceDao.getSingleServiceById(s1.getS_id());
						%>
						<tr>
							<th scope="row"><%=c1.getName()%></t0h>
							<td><%=ser.getStype()%></td>
							<td><%=ser.getCmodel()%></td>
							...
							<a href="BookServiceController?action"><td class=".text-danger" style="cursor: pointer">confirm</td></a>
							<td class=".text-danger"style="cursor: pointer">reject</td>
						</tr>
						<%
						}
						%>
					...
					<h2>
						<%
						out.print("no Pending services found ");
						%>
					</h2>
					<%
					}
